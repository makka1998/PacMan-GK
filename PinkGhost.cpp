...
void PinkGhost::getPacmanCoords(SDL_Rect *pacmanCoords) {
...
void PinkGhost::checkMovementInput(Map *map) {
    int x = floor(m_coordinates.x / TILE_SIZE);
    int y = floor(m_coordinates.y / TILE_SIZE);
    ...
    if (Mix_Playing(1) != 0) {
        ...
        if (m_startingDestinationReached) {
            ...
            //LEDIG TIL VENSTRE, og vi GÅR IKKE TIL HØYRE
            if (pathAvailable(*map).at(2) && m_direction != direction::RIGHT) {
                if (m_direction != direction::LEFT) {
                    m_last_direction = m_direction;
                }
                ...
                //LEDIG TIL HØYRE, og vi GÅR IKKE TIL VENSTRE
                ...
                //LEDIG OVER OSS, og vi GÅR IKKE NEDOVER
                ...
                //LEDIG UNDER OSS, og vi GÅR IKKE OPPOVER
            ...
            if (x < 15 && !(wayPointsReached.at(0))) {
                m_direction = direction::RIGHT;
                if (x == 15 - 1) {
                    wayPointsReached.at(0) = true;
                }
            } else if (y > 12 && !(wayPointsReached.at(1))) {
                m_direction = direction::UP;
                if (y == 12 + 1) {
                    wayPointsReached.at(1) = true;
                }
            } else if (x > wayPoints.at(2) && !(wayPointsReached.at(2))) {
                m_direction = direction::LEFT;
                if (x == wayPoints.at(2) + 1) {
                    wayPointsReached.at(2) = true;
                }
            } else if (y < wayPoints.at(3) && !(wayPointsReached.at(3))) {
                m_direction = direction::DOWN;
                if (y == wayPoints.at(3) - 1) {
                    wayPointsReached.at(3) = true;
                }
            } else if (x > wayPoints.at(4) && !(wayPointsReached.at(4))) {
                m_direction = direction::LEFT;
                if (x == wayPoints.at(4) + 1) {
                    wayPointsReached.at(4) = true;
                }
            } else if (y < wayPoints.at(5) && !(wayPointsReached.at(5))) {
                m_direction = direction::DOWN;
                if (y == wayPoints.at(5) - 1) {
                    wayPointsReached.at(5) = true;
                }
            } else {
                m_startingDestinationReached = true;
